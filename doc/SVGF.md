...
## 摘要
本文介绍了一种在计算全局光照时，用每像素一个采样的图重建出时间稳定的序列图。为了处理这种大噪点的图像输入，本文使用时间积累提高有效采样数量，并根据时间和空间上像素照度的方差进行分层的图像空间小波变换滤波[Dammertz et al. 2010]。这种层次结构可用于在不同尺度上，使用局部照度方差区分噪点和图像细节信息。
...
以下先对本算法的重建管线进行概述，包括光栅化和路径追踪输入及通过成分分离将识别噪点源的方法，还有重建滤波器及后处理步骤。第4节会详细介绍核心的重建算法。
...
为了对多孔小波滤波模板进行局部调整，我们利用时域累积来估计颜色亮度的逐像素方差，从而有效地检测噪点。关键的思想是，我们的重建应该避免改变小或没有噪点的区域的样本(例如，完全阴影区域)，而过滤更多的稀疏采样及噪点区域。通过分析不同时间的样本，过滤器检测一个特定样本的可靠性。注意，空间计算的方差仅能作为噪音程度的参考:噪音增加了方差，但无噪点区域也可能有较大的方差。
...
光栅化管线输出的GBuffer是无噪点的，因此我们可以使用GBuffer中的信息来定义识别公共表面的边缘停止函数。本文的实现参考了 Dammertz et al. [2010]的工作，即使用一个5×5的双边滤波器多孔小波分解来检测边缘。滤波模板的权重使用权重函数$w(p,q)$生成。
$$公式（4.1）$$
式中h=(1/16,1/4,3/8,1/4,1/16)为滤波核函数，$\omega$为模板覆盖的所有采样位置。

传统的权重函数w(p,q)一般使用几何信息和颜色信息来实现边缘停止的功能 [Dammertz et al . 2010]。本文使用的新的权值函数则使用了深度、世界空间法线以及滤波器输入的亮度作为参考属性。
$$公式（4.2）$$

在使用小波滤波之前，我们根据亮度方差的局部估计调整亮度边缘保留函数(见4.4节)，然后根据公式1对时域积分的颜色进行小波滤波，假设方差样本不相关，我们进行如下滤波:
$$公式（4）$$

我们使用滤波结果来引导下一级的多孔小波变换的边缘停止函数。我们的图像重建流程使用5级小波变换，等效于一个65×65像素的过滤器模板。

作为小波变换的一部分，我们从第一个小波迭代中输出滤波后的颜色作为我们的颜色历史，用于与未来帧的时间积分(参见图3)。虽然我们可以使用从其他小波级别过滤的颜色，但我们根据实验经验发现，使用第一个小波迭代进行时间集成可以最有效地改善了时间稳定性，也具有较小的空间过滤偏差。

## 4.4 边缘停止函数

鉴于实时渲染的要求，我们选择了三种边缘停止函数用于公式2中，以增强时间稳定性及鲁棒性，但同时也会增加空间偏差。

每个函数的样本拒绝能力由参数Rz、Rn和Rl控制。虽然这些参数的一系列值是有效的，但我们通过实验发现，Rz = 1、Rn = 128和Rl = 4在我们测试的所有场景中都运行良好。因此，我们不向用户公开这些参数。

深度。真实的场景通常在几何尺度上有很大的变化，特别是在开放的景观中。这使得全局边缘停止功能难以控制。因此，我们假设一个用于表面深度的局部线性模型，并测量裁剪空间内平面的差分。我们使用裁剪空间深度的屏幕空间偏导数估计局部深度模型。这给出了一个权重函数，定义为:
$$公式（4.3）$$
式中$\triangledown z$为裁剪空间内深度相对于屏幕空间坐标的梯度，e为小值，避免被零除。

法线。对于图像平面上p点处的输入的法线n(p)，我们使用世界空间上的法线的余弦函数:
$$公式（4.4）$$
之前在网格简化及抗锯齿算法方面的工作同样使用相似的方式来控制是否将两个表面合并在一起。

照度。亮度边缘停止函数的一个关键方面是它能够根据亮度的局部标准差重新归一化亮度，从而自动适应各种尺度。但是，在低采样数量区域的操作引入了我们对方差和标准差估计的不稳定性;这可能会导致一些图像错误问题。为了避免这些问题，我们使用3x3高斯核对方差图像进行预滤波，这大大提高了重构质量(见图5)。我们的亮度边缘停止函数变成:
$$公式（4.5）$$

对于高斯核函数g3x3和位于位置p的亮度li(p)，由于亮度方差随着后续滤波过程的迭代会趋于减小，因此每次迭代wi的影响都在增大，这种方法防止了过度模糊，保留了较多的图像细节。

## 5 结果与讨论

我们从各个方面来评估新的时空方差引导过滤器，这些指标包括最终图像质量，性能和时间稳定性。我们的评估关注于在1 spp图像上，因为我们的算法是针对这个采样率来设计。我们的原型使用OpenGL生成基础的可见性并实现SVGF。使用Optix来实现次级光线和阴影光线的追踪[Parker et al. 2010]。所有报告的性能结果都基于NVIDIA TITAN X (Pascal)显卡。

### 5.1 与已有研究成果的比较

我们在基于原始论文使用同一框架独立实现各种降噪算法，并对这些算法加以比较。对于一般的图像空间滤波，我们将其与Dammertz等人[2010]的边缘保留的a-trous小波滤波器(EAW)、Li等人[2012]的基于SURE的滤波器(SBF)以及Kalantari等人[2015]的基于学习的滤波器(LBF)进行比较。对于直接光照(阴影)，我们也比较了SVGF和Mehta等的轴对齐过滤器(AAF)等[2012]

我们的重建滤波器依赖于时间滤波，通过使用时间积累历史缓冲来获得更高的有效样本数量。一般来说，与之前的算法研究实现，SVGF可以获得更好的质量，无论是在视觉上(参见图6)还是在RMSE和SSIM方面(表1)。

此外，图10(a)显示，SVGF的帧间差异性少了10倍，提供了更高的时间稳定性。我们参考补充视频来进行视觉时间稳定性的比较。

对一些之前的算法我们额外增加了时间积累和时间抗锯齿流程。由于时间积累增加了有效样本计数，我们认为这种做法可以更公平地将之前的算法研究实现和时空方差引导滤波进行比较。虽然原始的EAW、SBF、LBF和AAF算法没有使用时间积累，但是我们看到了图像质量(表1)和时间稳定性的显著改善(图10(b))。为了清晰起见，我们将没有时间积累流程的实现称为EAWorig、SBForig和LBForiq。

AAF遵循Mehta等人[2012]的描述。因为我们的射线预算只允许一个阴影射线像素，我们收集一个固定空间窗口的最小和最大遮挡距离。我们对估计的遮挡距离和能见度输入应用时间滤波。

我们的SBF[2012]实现不执行任何自适应采样。由于每个像素只有一个样本，所以我们使用一个小的空间窗口估计颜色方差。我们遵循作者的建议，跳过无噪特征距离函数的重整化(如深度、法线、反照率)。我们在包含直接和间接光照的临时积累的解调样本上运行过滤器。

LBF[2015]使用样本代码提供的预训练网络权值。我们没有在我们的场景上进行再训练，所有的网络输入都是按照他们的论文中描述的那样计算的。我们为滤镜提供了经过时间滤波的颜色输入，包括直接和间接照明。该网络的输入包括直接和间接反照率，以及首次反弹的可见性信息，因此我们没有对样本进行解调。

由于我们的过滤器将经过空间过滤的样本作为时间积累的输入(参见图3)，因此从技术上讲，我们的空间过滤器并不使用与我们的比较过滤器相同的输入。然而，所有的结果都是确定地呈现的，所以所有的过滤器都运行在相同的样本位置上的相同路径跟踪样本集上。