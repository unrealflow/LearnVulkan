...
// layout(binding = 4 ,set = 0) uniform PrimNums{uint p[] } primNums;
struct Vertex
{
	vec3 pos;
	vec3 normal;
...
{
	vec4 d0 = vertices0.v[3 * index];
	vec4 d1 = vertices0.v[3 * index + 1];
...
void main()
{
	...
	hitValue.color = BRDF(mat0.m, mat0.m.baseColor, lightVector, -gl_WorldRayDirectionNV, normal, vec3(0.6, 0.8, 0.0), vec3(0.0, 0.6, 0.8));
	...
	normal = normalize(normal + mat0.m.roughness * noise(cam.lightPos.xy + origin.xy));
