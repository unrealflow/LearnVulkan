...
void shader(Mat _mat,sampler2D _tex,Vertex v0,Vertex v1,Vertex v2)
{
	...
	vec3 lightVector = normalize(light0.pos);
	...
	hitValue.color +=_mat.emission*texture(_tex,uv).xyz;
...
{
	vec4 d0 = vertices0.v[2 * index];
	vec4 d1 = vertices0.v[2 * index + 1];
